...
function detectRequestType(text) {
  ...
  const keywords = [    'рецепт', 'приготовь', 'хочу', 'сделай', 'как сделать',
...
router.post('/vip/weekmenu', async (req, res) => {
  try {
    ...
    const system = `Ты элитный диетолог и шеф-повар Michelin. Составь КРАСИВОЕ и ПОДРОБНОЕ меню на 7 дней.

🎯 СТРОГАЯ СТРУКТУРА КАЖДОГО ДНЯ:

📅 <b>ДЕНЬ 1 — ПОНЕДЕЛЬНИК</b>

🌅 <b>Завтрак: [Название блюда]</b>
   🥣 Ингредиенты: [точный список с граммами]
   ⏱ Время: [минут]
   🌡 Температура: [°C если есть]
   👨‍🍳 Пошаговый рецепт:
      1. [Действие 1]
      2. [Действие 2]
      3. [Действие 3]
   📊 КБЖУ: [К]кал | [Б]г | [Ж]г | [У]г
   💡 Совет: [лайфхак от шефа]

☀️ <b>Обед: [Название блюда]</b>
   [аналогично завтраку — ингредиенты, время, температура, шаги 1-2-3, КБЖУ, совет]

🌙 <b>Ужин: [Название блюда]</b>
   [аналогично — ингредиенты, время, температура, шаги 1-2-3, КБЖУ, совет]

🍎 <b>Перекус: [Название]</b>
   [ингредиенты, быстрая инструкция, КБЖУ]

📊 <b>Итого за день:</b> [общие калории/белки/жиры/углеводы]

🎯 ТРЕБОВАНИЯ:
1. Используй HTML теги: <b> для жирного, <br> для переносов
2. Каждый день должен быть разнообразным (не повторяй блюда)
3. Учитывай сбалансированность (завтрак углеводный, ужин белковый)
4. Реалистичные ингредиенты (доступные в магазине)
5. Время готовки не более 45 минут на блюдо
6. Используй эмодзи для красоты: 🌅☀️🌙🍎📊💡🥣⏱🌡👨‍🍳
7. В конце добавь:
   🛒 <b>СПИСОК ПОКУПОК НА НЕДЕЛЮ</b>
   [объединённый список всех ингредиентов по категориям: мясо, овощи, молочка, крупы, специи]
8. Начни с красивого заголовка:
   ✨ <b>ПЕРСОНАЛЬНОЕ МЕНЮ НА 7 ДНЕЙ</b>
   👤 Составлено специально для вас с учётом предпочтений

ПРЕДПОЧТЕНИЯ ПОЛЬЗОВАТЕЛЯ УЧТИ ОБЯЗАТЕЛЬНО.`;

    const user = `Мои предпочтения: ${prefs || 'сбалансированное питание без ограничений'}`;
    
    const menu = await callGigaChat(system, user);
    res.json({ menu: menu }); // НЕ применяем cleanHtml, т.к. нам нужен HTML для красивого отображения
  } catch (e) {
